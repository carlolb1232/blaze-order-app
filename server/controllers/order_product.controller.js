...
module.exports.getOrderProductsFromOrder = async (req, res) => {
  try {
    ...
    const order = await Order.findById(idOrder).populate({
      ...
      .exec();
    res.json({ message: "", orderProducts: order.order_items })
  } catch (err) {
    res.json({ message: 'Ocurrio un error', errors: err.errors })
  }
}

module.exports.deleteOneOrderProduct = async (req, res) => {
  try {
    const { id } = req.params
    const orderProductDeleted = await OrderProduct.findByIdAndDelete({_id:id})
    res.json({message:"", orderProductDeleted:orderProductDeleted})
  } catch (err) {
    res.json({ message: 'Ocurrio un error', errors: err.errors })
  }
}
